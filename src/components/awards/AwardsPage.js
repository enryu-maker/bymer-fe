...
function AwardCard({ award }) {
  return (
    <article className="bg-white border border-[#e5e7eb] p-4 sm:p-5 flex flex-col items-center gap-4 shadow-[0_1px_3px_rgba(0,0,0,0.05)] border-t-[3px] border-t-[#C75550]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {awards.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {awards.map((award) => (
              <AwardCard key={award.id ?? award.name} award={award} />
            ))}
          </div>
        ) : (
